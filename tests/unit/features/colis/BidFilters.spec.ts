...
describe('BidFilters', () => {
  it('renders all status filter tabs', async () => {
    ...
    expect(wrapper.find('[data-test="filter-status-PAYMENT_ESCROWED"]').exists()).toBe(true)
    ...
    expect(wrapper.find('[data-test="filter-status-COMPLETED"]').exists()).toBe(true)
    expect(wrapper.find('[data-test="filter-status-REJECTED"]').exists()).toBe(true)
    expect(wrapper.find('[data-test="filter-status-CANCELLED"]').exists()).toBe(true)
  })

  it('emits update:statusFilter when a tab is clicked', async () => {
    const wrapper = await mountBidFilters()
    await wrapper.find('[data-test="filter-status-PAYMENT_ESCROWED"]').trigger('click')
    expect(wrapper.emitted('update:statusFilter')?.[0]).toEqual(['PAYMENT_ESCROWED'])
  })
  ...
  it('applies active styling to the currently selected status tab', async () => {
    const wrapper = await mountBidFilters({
      modelValue: { ...defaultFilters, statusFilter: 'PAYMENT_ESCROWED' },
    })
    const activeTab = wrapper.find('[data-test="filter-status-PAYMENT_ESCROWED"]')
